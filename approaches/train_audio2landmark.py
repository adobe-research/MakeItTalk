...
import time
from dataset.audio2landmark.audio2landmark_dataset import Audio2landmark_Dataset
from models.model_audio2landmark import *
from util.utils import Record, weight_init, get_n_params, try_mkdir, vis_landmark_on_plt
import random
...
class Audio2landmark_model():
    ...
    def __train_l2_pos_gan_loss2__(self, fls, aus, embs, face_id):
    ...
    #
        ...
        mean_out1 = 0.5 * fl_dis_pred[:, 49:54] + 0.5 * fl_dis_pred[:, index1]
        mean_out2 = 0.5 * fl_dis_pred[:, 49:54] + 0.5 * fl_dis_pred[:, index1]
        fl_dis_pred[:, 49:54] = mean_out1
        fl_dis_pred[:, index1] = mean_out2
        ...
        residual_face_id = baseline_face_id  # + fl_dis_pred[0, :]
        # ''' CALIBRATION '''
        ...
        # baseline_pred_fls[:, 48 * 3 + 1::3] /= torch.sqrt(torch.abs(baseline_pred_fls[:, 48 * 3 + 1::3]))
        # my = torch.mean(baseline_pred_fls[:, 48 * 3 + 1::3], dim=1, keepdim=True)
        # baseline_pred_fls[:, 48 * 3 + 1::3] = (baseline_pred_fls[:, 48 * 3 + 1::3] - my) * self.opt_parser.amp_lip_y \
        #                                       + my * ((self.opt_parser.amp_lip_y - 1.0) * 0.75 + 1)
        # baseline_pred_fls[:, 48 * 3 + 1::3] += 0.04
        fl_dis_pred += baseline_pred_fls

        return fl_dis_pred, face_id[0:1, :]

    def __train_pass__(self, au_emb=None):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                # load saved emb
                if(au_emb is None):
                ...
                for j in range(0, inputs_fl.shape[0], seg_bs):
                    ...
                    # input_face_id = torch.tensor(registered_landmarks[:, 0:3].reshape(1, 204), requires_grad=False, dtype=torch.float).to(device)

                    fl_dis_pred_pos, input_face_id = \
                        self.__train_l2_pos_gan_loss2__(inputs_fl_segments, inputs_au_segments, inputs_emb_segments,
                                                           input_face_id)
